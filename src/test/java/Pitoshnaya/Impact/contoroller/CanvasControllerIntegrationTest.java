...
public class CanvasControllerIntegrationTest extends BaseControllerIntegrationTest {
    ...
    void testDrawIsValidDrawRequest(DrawRequest request) {
        var response
            = sendPut(url, request, Void.class);
    ...
    void testDrawIsInvalidDrawRequest(DrawRequest request, ValidationErrorResponse expected) {
        var response
            = sendPut(url, request, ValidationErrorResponse.class);

        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        assertThat(response.getBody().violations()).containsExactlyInAnyOrderElementsOf(expected.violations());
    }
    ...
    public Stream<Arguments> invalidRequestProvider() {
        ...
        return Stream.of(
            ...
            Arguments.of(
                new DrawRequest(1, 1, "red"),
                new ValidationErrorResponse(List.of(invalidColor))
            ),
            Arguments.of(
